...
core_non_core_model_path = 'mining_lithology_model_core_none_20240229.tflite'
...
# Function to classify core/non-core
def classify_core_non_core(image):
    ...
    # Set input tensor
    input_details = core_non_core_interpreter.get_input_details()
    core_non_core_interpreter.set_tensor(input_details[0]['index'], image_array)

    # Run inference
    core_non_core_interpreter.invoke()

    # Get output tensor
    output_details = core_non_core_interpreter.get_output_details()
    output = core_non_core_interpreter.get_tensor(output_details[0]['index'])

    # Get predicted class
    predicted_class_index = np.argmax(output[0])
    predicted_label_name = core_non_core_classes[predicted_class_index]
...
if option == "Upload Image":
    ...
    if uploaded_image is not None:
        ...
        predicted_label_name = classify_core_non_core(image)
        ...
        if predicted_label_name == "Core":
            ...
            col1.write(f"**Predicted Lithology: [{predicted_label_name}]({class_information[predicted_label_name]})**", unsafe_allow_html=True)

            # Display the image in the first column
            col1.image(image, caption=f"Predicted Lithology: {predicted_label_name}", width=400)
            ...
            if predicted_label_name in physical_tests_info:
                ...
                col2.write(further_investigation_info[predicted_label_name])
            ...
            st.write("Please provide a core image. The provided image is classified as non-core.")
